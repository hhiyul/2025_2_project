...
# ------------------------------------------------------------
# 3) 아주 단순한 CNN (BatchNorm/Dropout/정규화 없음)
# ------------------------------------------------------------
...
all_labels = [int(final_dataset["train"][i]["label"]) for i in range(len(final_dataset["train"]))]
indices = list(range(len(final_dataset["train"])) )
skf = StratifiedKFold(n_splits=K, shuffle=True, random_state=42)

fold_accs = []  # ★ 폴드별 에폭별 val_acc 저장용
